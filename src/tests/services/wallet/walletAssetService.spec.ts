...
import { addCryptoWalletClient, createDepositWithdrawal, deleteAssetWallet, getAllAssetsInOrgForAddWalletClient, getAllAssetsWalletClient, rebalanceWallet, updateAssetWalletInformations } from "@/services/wallet/walletAssetService";
...
describe('walletAssetService', () => {
    ...
    describe('createDepositWithdrawal', () => {
      ...
      it('should throw an error when deposit or withdrawal fails', async () => {
        (instance.post as jest.Mock).mockRejectedValue(new Error('Invalid deposit data'));
  
        await expect(createDepositWithdrawal(-500, '123', 'INVALID', true))
    ...
    describe('deleteAssetWallet', () => {
      ...
      it('should throw an error when deleting a non-existent asset', async () => {
        (instance.delete as jest.Mock).mockRejectedValue(new Error('Asset not found'));
  
        await expect(deleteAssetWallet('123', '999'))
    ...
    describe('rebalanceWallet', () => {
      ...
      it('should throw an error when rebalancing wallet fails', async () => {
        ...
        await expect(rebalanceWallet('INVALID'))
