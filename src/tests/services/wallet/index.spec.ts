...
describe('walletAssetService', () => {
  ...
  it('should handle error when fetching assets', async () => {
    ...
    try {
      const result = await getAllAssetsWalletClient('123');
      expect(result).toBeFalsy(); // Aceita null ou undefined
    } catch (error) {
  ...
  it('should handle error when updating asset wallet information with invalid data', async () => {
    ...
    try {
      ...
      expect(result).toBeFalsy(); // Aceita false, null ou undefined
    } catch (error) {
      expect(error).toBeInstanceOf(Error);
      if (error instanceof Error) {
...
describe('walletInfoService', () => {
  ...
  it('should request start wallet', async () => {
    ...
    expect(instance.put).toHaveBeenCalledWith('wallet/123/startWallet', {
      customDate: '2023-01-01',
    });
  });
  ...
  it('should handle error when updating asset wallet information with invalid data', async () => {
    ...
    try {
      ...
      expect(error).toMatchObject({ message: 'Invalid data' });
    }
  });
  ...
  it('should handle error when adding crypto asset with invalid data', async () => {
    ...
    try {
      ...
      expect(error).toMatchObject({ message: 'Invalid asset data' });
    }
  });
  ...
  it('should handle error when creating deposit or withdrawal with invalid data', async () => {
    ...
    try {
      const result = await createDepositWithdrawal(-500, '123', 'INVALID', true);
      expect(result).toBeUndefined();
    } catch (error) {
      expect(error).toMatchObject({ message: 'Invalid deposit data' });
    }
  });
  ...
  it('should handle error when deleting non-existent asset', async () => {
    ...
    try {
      const result = await deleteAssetWallet('123', '999');
      expect(result).toBeUndefined();
    } catch (error) {
      expect(error).toMatchObject({ message: 'Asset not found' });
    }
  });
  ...
  it('should handle error when rebalancing wallet with invalid data', async () => {
    ...
    try {
      const result = await rebalanceWallet('INVALID');
      expect(result).toBeUndefined();
    } catch (error) {
      expect(error).toMatchObject({ message: 'Rebalance failed' });
    }
  });
  ...
  it('should handle error when fetching available assets', async () => {
    ...
    try {
      ...
      expect(error).toMatchObject({ message: 'Failed to fetch assets' });
    }
  });
});