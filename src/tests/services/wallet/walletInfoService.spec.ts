...
describe('walletInfoService', () => {
    ...
    describe('getInfosCustomer', () => {
      ...
      it('should throw an error when fetching customer information for a non-existent wallet', async () => {
        (instance.get as jest.Mock).mockRejectedValue(new Error('Wallet not found'));
  
        await expect(getInfosCustomer('non-existent-wallet')).rejects.toThrow('Wallet not found');
        expect(instance.get).toHaveBeenCalledWith('wallet/non-existent-wallet/infos');
      });
    });
    ...
    describe('registerWalletForCustomer', () => {
      it('should register a wallet for a customer', async () => {
        ...
        const result = await registerWalletForCustomer(
          '123', 'USD', 1000, 10, true, 'Low', true, 5, 'cuid1', 'exchange1', 'manager1'
        );
      ...
      it('should throw an error when registering a wallet with missing required fields', async () => {
        (instance.post as jest.Mock).mockRejectedValue(new Error('Missing required fields'));
        await expect(registerWalletForCustomer(
          '', 'USD', 1000, 10, true, 'Low', true, 5, 'cuid1', 'exchange1', 'manager1'
    ...
    describe('requestCloseWallet', () => {
      ...
      it('should throw an error when requesting to close a non-existent wallet', async () => {
        (instance.put as jest.Mock).mockRejectedValue(new Error('Wallet not found'));
  
        await expect(requestCloseWallet('non-existent-wallet', { customDate: '2023-01-01' }))
          .rejects.toThrow('Wallet not found');
  
        expect(instance.put).toHaveBeenCalledWith('wallet/non-existent-wallet/closeWallet', {
          customDate: '2023-01-01',
        });
      });
    });
    ...
    describe('getWalletKpis', () => {
      it('should fetch wallet KPIs', async () => {
        ...
        expect(result).toEqual(mockData);
        expect(instance.get).toHaveBeenCalledWith('wallet/123/kpis', { params: { period: 'monthly' } });
      });
  
      it('should throw an error when fetching wallet KPIs with an invalid period', async () => {
        (instance.get as jest.Mock).mockRejectedValue(new Error('Invalid period parameter'));
  
        await expect(getWalletKpis('123', 'invalid-period')).rejects.toThrow('Invalid period parameter');
        expect(instance.get).toHaveBeenCalledWith('wallet/123/kpis', { params: { period: 'invalid-period' } });
      });
    });
  });
