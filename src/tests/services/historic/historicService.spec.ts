...
describe('getWalletHistoric', () => {
  ...
  it('should throw an error when fetching wallet historic fails', async () => {
    (instance.get as jest.Mock).mockRejectedValue(new Error('Failed to fetch historic'));

    await expect(getWalletHistoric('wallet-123')).rejects.toThrow('Failed to fetch wallet historic');
  });
});
