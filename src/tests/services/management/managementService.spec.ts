...
describe('managementService', () => {
  ...
  describe('addCryptoOrg', () => {
    it('should add a new crypto asset to organization', async () => {
      ...
      expect(instance.post).toHaveBeenCalledWith('management/asset', { idCmc: [1, 2] });
    });
  ...
  describe('registerNewCustomer', () => {
    it('should register a new customer', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(instance.post).toHaveBeenCalledWith('management/costumer', { name: 'John Doe', email: 'john@example.com' });
    });
  });
  ...
  describe('confirmContactClient', () => {
    it('should confirm contact for a client', async () => {
      ...
      expect(instance.patch).toHaveBeenCalledWith('management/contact', { walletUuid: 'wallet-123' });
    });
    ...
    it('should handle error when confirming contact for a client', async () => {
      ...
      await expect(confirmContactClient('wallet-123')).rejects.toThrow('Contact error');
    });
  });
  ...
  describe('updateWallet', () => {
    it('should update wallet information', async () => {
      ...
      expect(instance.put).toHaveBeenCalledWith('management/wallet/wallet-123', expect.any(Object));
    });
  });
