...
describe('CustomersOrganization', () => {
  ...
  describe('DisableCustomerModal', () => {
    ...
    it('handles customer disable process correctly', async () => {
      ...
      ;(managementService.deleteCustomer as jest.Mock).mockResolvedValue(
        undefined,
      )
      ...
      expect(mockOnOpenChange).toHaveBeenCalledWith(false)
      expect(managementService.deleteCustomer).toHaveBeenCalledWith(
        mockCustomerUuid,
      )
    })
    ...
    it('handles disable error correctly', async () => {
      ...
      ;(managementService.deleteCustomer as jest.Mock).mockRejectedValue(
  ...
  describe('ProfileTab', () => {
    it('renders the name, email, and phone inputs profile tab', () => {
      render(
        <Dialog open={true} onOpenChange={() => {}}>
          <ProfileTab
            setPhoneCountry={function (): void {
      ...
      const phoneText = screen.queryAllByText('Phone')
      const phoneInput = phoneText[1]
  ...
  describe('RegisterCustomerModal', () => {
    it('renders the name, email, and phone inputs register customer', () => {
  ...
  const mockWalletTabProps = {
    ...
    emailExchangeRef: {
      current: { value: 'exchange@example.com' },
    } as React.RefObject<HTMLInputElement>,
    accountPasswordRef: {
      current: { value: 'exchangepass123' },
    } as React.RefObject<HTMLInputElement>,
    manager: 'manager-1',
    setManager: jest.fn(),
    ...
    isUpdateWithBaseWallet: false,
    setIsUpdateWithBaseWallet: jest.fn(),
  }
  ...
  describe('WalletTab', () => {
    ...
    it('renders a message when isWallet is false', () => {
      ...
      expect(
        screen.getByText(
          /please create a wallet first before filling these details\./i,
        ),
      ).toBeInTheDocument()
    })
    ...
    describe('WalletTab Component', () => {
      ...
      it('allows clearing and updating the Performance Fee input', async () => {
        ...
        waitFor(() => {
          ...
          expect(mockWalletTabProps.setPerformanceFee).toHaveBeenCalledWith(
            '20',
          )
        })
      })
