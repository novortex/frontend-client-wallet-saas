...
import { CircleAlert, Check, DollarSign, Calendar, Wallet, BarChartBigIcon, PhoneCall } from 'lucide-react'
...
export function Infos() {
  ...
  return (
    <div className="bg-white dark:bg-transparent p-10">
                ...
                walletCommission.map((item) => (
                  <div key={item.name}>
                    <p className="text-[#959CB6] mr-5">
                      {item.name} <span className="text-gray-300">({item.commission}%)</span>
                    </p>
                  </div>
                ))
              ) : (
      ...
      <ClientsInfoModal isOpen={isModalOpen} onClose={closeModal} name={walletI.user.name} email={walletI.user.email} phone={walletI.user.phone} />
      <ExchangeInfoModal
      ...
      />
      <ConfirmContactModal isOpen={isModalContactOpen} onClose={closeModalContact} />
    </div>
  )
}
