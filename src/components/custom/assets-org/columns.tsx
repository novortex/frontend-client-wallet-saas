      ...
      <DialogContent className="border-0 text-black dark:bg-[#1C1C1C] dark:text-white">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <div className="mt-4 rounded bg-orange-100 p-4 text-orange-800 dark:bg-yellow-900 dark:text-yellow-200">
...
export const columnsAssetOrg: ColumnDef<AssetOrgs>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => (
      <div
        className="cursor-pointer select-none rounded p-2 text-center transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
    ...
    sortingFn: (rowA, rowB) => {
      const getNumericValue = (appearances: string) => {
        const match = appearances.match(/(\d+)/)
        return match ? parseInt(match[1], 10) : 0
      }

      const valueA = getNumericValue(rowA.original.appearances)
      const valueB = getNumericValue(rowB.original.appearances)
