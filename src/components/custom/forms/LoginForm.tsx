...
const LoginForm: React.FC<LoginFormProps> = ({ onSubmit, emailRef, passwordRef, isLoading }) => {
    return (
        <form onSubmit={onSubmit}>
            <InputField id="email" label="Email" type="email" ref={emailRef} required />
            <InputField id="password" label="Password" type="password" ref={passwordRef} required />
            <ForgotPassword />
...
const InputField = React.forwardRef<HTMLInputElement, InputFieldProps>(
    ({ id, label, type = 'text', ...props }, ref) => (
        <div className="w-full h-1/2 flex flex-col justify-center items-start gap-2.5">
            <Label htmlFor={id} className="text-white text-lg">{label}</Label>
            <Input
                id={id}
                type={type}
                placeholder={label}
